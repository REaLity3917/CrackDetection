...
def calculate_crack_metrics(binary_image):
    ...
    crack_widths = [prop.major_axis_length for prop in props]
    ...
    # 计算裂缝间距
    crack_spacing = np.mean([prop.minor_axis_length for prop in props])
    ...
    # # 计算裂缝密度
    # crack_density = num_features / (binary_image.shape[0] * binary_image.shape[1])
    ...
    # 计算裂缝长度变化率
    crack_length_change_rate = (crack_length - np.sum(binary_image)) / np.sum(binary_image)
    
    # 计算裂缝面积变化率
    crack_area_change_rate = (crack_area - np.sum(binary_image)) / np.sum(binary_image)
    
    # 计算裂缝形状因子
    crack_shape_factor = avg_crack_shape_factor
    ...
    index_basic_metric = calculate_basic_metrics(orign_binary_image,0.25,0.25,0.25,0.25) ##该指标计算相当主观，需要根据情况微调
    ...
    return [
        ...
        crack_length_change_rate,
        # crack_area_change_rate,
        crack_shape_factor,
        crack_distribution_uniformity,  ##似乎是正向指标
...
# # 读取二值化图像
# binary_image = cv2.imread('crack_binary_image.png', cv2.IMREAD_GRAYSCALE)
